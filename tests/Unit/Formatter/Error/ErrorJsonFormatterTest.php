...
#[CoversClass(ErrorJsonFormatter::class)]
class ErrorJsonFormatterTest extends TestCase
{
    #[DataProviderExternal(FormatterDataProvider::class, 'getAnalyseValueObject')]
    public function testOutputIsMinifiedJsonAndReturnsError(AnalyseValueObject $except): void
    {
        $formatter = new ErrorJsonFormatter();
        ...
        self::assertStringNotContainsString('<promote>', $json);
        self::assertStringNotContainsString('</promote>', $json);
        ...
        self::assertSame('Resource x must be a final class', $decoded['errors'][0]['message']);
